...
var _utils2Js = require("./utils2.js");
...
const scroll = ()=>{
    const gridObj = (0, _utils2Js.getGrid)(mergedItems.map((item)=>item.element));
    const rowMapping = {
        even: {
            skewX: 10,
            xPercent: 2,
    ...
    [
        ...
        gridObj.rows(type).flat().forEach((row)=>{
            gsap.timeline({
                ...
                xPercent: 0,
                skewX: 0
            }, 0).fromTo(row.querySelector(".column__item-img"), {
                scaleY: 1.2
            }, {
                scaleY: 1
            }, 0);
...
const getGrid = (selector)=>{
    let elements = gsap.utils.toArray(selector), bounds, getSubset = (axis, dimension, alternating, merge)=>{
        let a = [], subsets = {}, onlyEven = alternating === "even", p;
        bounds.forEach((b, i)=>{
            let position = Math.round(b[axis] + b[dimension] / 2), subset = subsets[position];
            subset || (subsets[position] = subset = []);
            subset.push(elements[i]);
        });
        for(p in subsets)a.push(subsets[p]);
        if (onlyEven || alternating === "odd") a = a.filter((el, i)=>!(i % 2) === onlyEven);
        if (merge) {
            let a2 = [];
...
},{}]},["bVI4s","dfBCx"], "dfBCx", "parcelRequire2524")

//# sourceMappingURL=index2.4e262f69.js.map
